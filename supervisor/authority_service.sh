#! /bin/sh
runFile=bird-authority-1.0.0.jar
#启动方法
start(){
	sleep 30
	ps -ef|grep "bird-task-server"|grep -v "grep"|awk '{print $2}'|while read bird-task-server_pid
        ...
        exec /usr/local/jdk1.7.0_79/bin/java -Xms128m -Xmx2048m -Duser.timezone=GMT+08 -jar lib/$runFile >> /opt/logs/bird-authority.log
}
#停止方法
stop(){
